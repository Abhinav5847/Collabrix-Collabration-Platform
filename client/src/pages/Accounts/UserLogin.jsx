...
const UserLogin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (loginUser.fulfilled.match(resultAction)) {
      ...
      // Fetch profile immediately so the UI (like Navbars) has the user data instantly
      const userId = resultAction.payload.user?.id || resultAction.payload.id;
      if (userId) {
        await dispatch(fetchUserProfile(userId));
      }

      // Short delay so the user actually sees the toast before redirecting
      setTimeout(() => {
        navigate("/");
      }, 1000);
    } else {
